...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        # NEW: Agent type embeddings
        # 6 classes: Pedestrian, Biker, Skater, Cart, Car, Bus
        self.agent_type_embedding = nn.Embedding(
            num_embeddings=6,
            embedding_dim=16
        )
        
        # NEW: Class-specific environment means for counterfactuals
        # Buffer (not trained, just updated with running average)
        self.register_buffer(
            'env_mean_per_class',
            torch.zeros(6, graph_lstm_hidden_size)
        )
        
        # NEW: Learnable lambda (intervention strength) per class
        # Higher values = stronger bias correction
        self.lambda_per_class = nn.Parameter(
            torch.tensor([0.3, 0.5, 0.7, 0.7, 0.4, 0.5])
            # [Pedestrian, Biker, Car, Bus, Skater, Cart]
        )

        # MODIFIED: Update pred_lstm_hidden_size to include agent type embedding
        self.pred_lstm_hidden_size = (
            self.traj_lstm_hidden_size + 
            self.graph_lstm_hidden_size + 
            16 +  # NEW: agent type embedding dimension
            noise_dim[0]
        )
    ...
    def forward(
        ...
        obs_traj_pos,
        seq_start_end,
        agent_class_ids,  # NEW: Agent class IDs
        ...
        if training_step == 1 or training_step == 2:
            ...
            # MODIFIED COUNTERFACTUAL SECTION WITH CLASS-CONDITIONING
          
            
            # Counterfactual: zero out trajectory history
            traj_lstm_hidden_states_c = torch.zeros_like(traj_lstm_hidden_states[-1])
            
            # NEW: Get agent type embeddings
            type_embeddings = self.agent_type_embedding(agent_class_ids)  # [batch, 16]
            
            # NEW: Use class-specific environment means for counterfactual
            graph_lstm_hidden_c = self.env_mean_per_class[agent_class_ids]  # [batch, graph_hidden]
            
            # NEW: Update running environment means per class during training
            if self.training:
                with torch.no_grad():
                    for cls in range(6):  # 6 agent classes
                        mask = (agent_class_ids == cls)
                        if mask.any():
                            cls_graph_mean = graph_lstm_hidden_states[-1][mask].mean(dim=0)
                            # Exponential moving average with momentum 0.99
                            self.env_mean_per_class[cls] = (
                                0.99 * self.env_mean_per_class[cls] + 
                                0.01 * cls_graph_mean
                            )
            
            # Factual: trajectory + social context + agent type
            encoded_before_noise_hidden = torch.cat(
                (traj_lstm_hidden_states[-1], graph_lstm_hidden_states[-1], type_embeddings),
                dim=1
            )
            
            # Counterfactual: zero trajectory + class-specific environment + agent type
            encoded_before_noise_hidden_c = torch.cat(
                (traj_lstm_hidden_states_c, graph_lstm_hidden_c, type_embeddings),
                dim=1
            )
            
            # Add noise to both factual and counterfactual
            ...
            if self.training:
                for i, input_t in enumerate(
                    ...
                    # NEW: Apply class-specific lambda values
                    # lambda controls intervention strength per agent class
                    lambda_vals = self.lambda_per_class[agent_class_ids].unsqueeze(-1)  # [batch, 1]
                    pred_lstm_hidden_out = (
                        pred_lstm_hidden - 
                        lambda_vals * (pred_lstm_hidden - pred_lstm_hidden_c)
                    )
